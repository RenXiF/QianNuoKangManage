						...
						<view class="block_noe_blck_bk_two">
								<view class="content_list_one">
									<view class="flex_columns" v-if="menuLists">
										<view class="content_list_one_bl flex-center flex_rows" v-for="(item,index) in menuLists" :key='index' v-if="sorttid2==item.status">
											<text>{{ index+1 }}</text>
											<text>{{ item.id }}</text>
											...
											<view class="content_list_one_bl_default_user flex-center">
												<button type="default" @click="deliverdizhi(item.addressid)">查询</button>
...
export default {
	...
	methods: {
		...
		userquery(){
			if (this.querySearch != null) {
				...
				this.http.getApi('/Companyorder/getCompanyOrder',{orderNo:this.querySearch}, 'get', this.userlist.storeOpendid).then(res => {
					...
					if (res.status != this.sorttid2) {
						this.sorttid2 = res.status;
		...
		userlistdata(e){
			this.utils.showloading();
			this.http.getApi('/Companyorder/CompanyOrder', {status:e,pageNum:this.pageNum, pageSize:10}, 'get', this.userlist.storeOpendid).then(res => {
				uni.hideLoading();
				console.log(res);
